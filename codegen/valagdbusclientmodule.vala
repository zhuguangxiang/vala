...
public class Vala.GDBusClientModule : GDBusModule {
	...
	void generate_get_property_function (ObjectTypeSymbol sym) {
		...
		foreach (Property prop in sym.get_properties ()) {
			...
			ccode.add_assignment (new CCodeIdentifier ("dbus_property_name"), new CCodeConstant.string ("\"%s\"".printf (get_dbus_name_for_member (prop))));
			ccode.add_break ();
		}
		...
		var get_cached_property_function = new CCodeFunctionCall (new CCodeIdentifier ("g_dbus_proxy_get_cached_property"));
		...
		ccode.open_if (new CCodeBinaryExpression (CCodeBinaryOperator.INEQUALITY, new CCodeIdentifier ("variant"), new CCodeConstant ("NULL")));

		var dbus_gvariant_to_gvalue = new CCodeFunctionCall (new CCodeIdentifier ("g_dbus_gvariant_to_gvalue"));
	...
	void generate_set_property_function (ObjectTypeSymbol sym) {
		...
		ccode.add_declaration ("GVariant *", new CCodeVariableDeclarator ("variant", null));
		ccode.add_declaration ("const gchar *", new CCodeVariableDeclarator.zero ("dbus_property_name", new CCodeConstant ("NULL")));
		ccode.add_declaration ("const GVariantType *", new CCodeVariableDeclarator.zero ("dbus_variant_type", new CCodeConstant ("NULL")));
		...
		foreach (Property prop in sym.get_properties ()) {
			...
			variant_type_cast.add_argument (new CCodeConstant.string ("\"%s\"".printf (prop.property_type.get_type_signature (prop))));
		...
		var dbus_gvalue_to_gvariant = new CCodeFunctionCall (new CCodeIdentifier ("g_dbus_gvalue_to_gvariant"));
		...
		arg_variant.add_argument (new CCodeIdentifier ("dbus_variant_type"));
		...
		dbus_proxy_call.add_argument (new CCodeConstant.string ("\"org.freedesktop.DBus.Properties.Set\""));
